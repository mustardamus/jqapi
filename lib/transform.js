...
module.exports = class Transform {
  ...
  getTopDesc () {
    if (this.$('entries').length) {
      return this.$('entries > desc').html()
    } else {
      return this.$('entry > desc').html()
    }
  }
  ...
  getSignatureArguments ($signature) {
    return $signature.find('> argument').map((i, el) => {
      ...
      let retObj = {
        name: $arg.attr('name'),
        desc: $arg.children('desc').html(),
        types: []
      }

      if ($arg.attr('type')) {
        retObj.types.push($arg.attr('type'))
      } else {
        retObj.types = $arg.find('type').map((i, el) => {
          return this.$(el).attr('name')
        }).get()
      }

      if ($arg.attr('optional') === 'true') {
        retObj.optional = true
      }

      if (retObj.types[0].toLowerCase() === 'function') {
        retObj.callback = this.getArgumentCallbackMetas($arg)
      }
